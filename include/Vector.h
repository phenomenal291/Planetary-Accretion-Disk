// For convenience, instead of reimplement a whole vector class, I choose this one which is implemented by Mahendra Dani.
//source: https://github.com/MahendraDani/vector
#pragma once
...
class Vector{
  ...
  public:
    ...
    const Vector<T> operator-(const Vector<T>& other) const;
    const Vector<T> operator*(const Vector<T>& other) const;
    const Vector<T> operator+(const T value) const;
    const Vector<T> operator-(const T value) const;
    const Vector<T> operator*(const T value) const;
    const Vector<T> operator/(const T value) const;
    // vector divided by every scalar, not vector
    const Vector<T> operator/(const Vector<T>& other) const = delete; // delete this operator to avoid confusion
...
template<typename T>
const Vector<T> Vector<T>::operator+(const T value) const{
